...
\node[state, fill=green!20] (active) at (4.6,0) {ACTIVE};
\node[state, fill=orange!20] (dissolving) at (9.2,0) {DISSOLVING};
\node[state, fill=red!20] (dissolved) at (13.8,0) {DISSOLVED};

\draw[arr] (pending) -- (active);
\node[lab] at (2.3,0.42) {管理员审批通过};

\draw[arr] (active) -- (dissolving);
\node[lab] at (6.9,0.42) {申请解散};

\draw[arr] (dissolving) -- (dissolved);
\node[lab] at (11.5,0.42) {通过解散审批 / 冷静期结束};

\draw[arr] (dissolving.south) .. controls (9.2,-1.5) and (5.0,-1.5) .. (active.south);
\node[lab] at (7.1,-1.25) {撤回 / 驳回};

\draw[arr] (dissolved.south) .. controls (13.8,-2.4) and (5.4,-2.4) .. (active.south east);
\node[lab] at (10.0,-2.1) {恢复社团};
\end{tikzpicture}
\caption{社团状态流转实现}
...
\subsubsection{活动奖励结算与成员积分档案}

在活动模块之外，系统还将成员激励能力收敛到社团成员关系中实现。其核心数据结构不是单独维护“全站积分账户”，而是直接在 \texttt{t\_member} 表中保存成员在该社团内的当前积分，并以 \texttt{t\_member\_point\_record} 记录每次增减流水、来源类型、来源业务单据、操作人和结算后余额。这样既能满足成员档案页对当前积分与流水记录的联合展示，也能保证积分口径天然受社团边界约束。

活动奖励结算由 \texttt{MemberArchiveServiceImpl} 统一处理。活动创建和编辑阶段可配置 \texttt{reward\_points}、\texttt{need\_attendance}、\texttt{settlement\_status} 与 \texttt{settled\_at} 等字段；当活动状态已变为 \texttt{ENDED} 后，社团管理员可在活动管理页调用 \texttt{/activities/\{id\}/settle-rewards} 发起结算。若活动要求签到，则以签到记录作为奖励对象；否则以未取消报名记录作为奖励对象。结算过程会先锁定活动与相关成员记录，再根据旧流水回滚历史发放结果，最后按最新口径重建积分流水，因此前端看到的“重新结算”并不是简单追加积分，而是一次可重算的校正过程。

除活动自动奖励外，系统也保留了面向社团管理员的人工调分接口 \texttt{/clubs/\{id\}/members/\{userId\}/points/adjust}，用来处理补录、纠错等小规模运营场景。为避免异常数据扩散，服务层在调分和重算时都会校验成员积分结果不得为负数；若重算后出现负值风险，则直接拒绝本次操作并提示先处理历史流水。由此，活动结算、人工调整与成员档案查询被统一纳入同一套成员激励模型中。
...
\node[box, fill=blue!15] (producer) at (0,2.4) {业务模块\\Club / Recruit / Finance};
\node[box, fill=orange!20] (exchange) at (5.0,2.4) {Topic Exchange\\notification.topic};
\node[box, fill=yellow!20] (queue) at (10.0,2.4) {Queue\\notification.queue};
\node[box, fill=green!20] (consumer) at (15.0,2.4) {NotificationConsumer};
\node[box, fill=gray!15] (ui) at (10.0,0.0) {前端消息中心\\轮询未读数/列表};
\node[box, fill=purple!15] (db) at (15.0,0.0) {t\_notification\\站内通知表};

\draw[arr] (producer) -- (exchange);
\node[lab] at (2.5,2.78) {notification.club.approved 等};

\draw[arr] (exchange) -- (queue);
\node[lab] at (7.5,2.78) {Routing Key 匹配};

\draw[arr] (queue) -- (consumer);
\node[lab] at (12.5,2.78) {监听消费};

\draw[arr] (consumer.south) -- (db.north);
\node[lab] at (15.6,1.2) {落库};

\draw[arr] (db.west) -- (ui.east);
\node[lab] at (12.5,0.42) {查询通知列表};

\draw[arr] (ui.south east) .. controls (11.4,-1.0) and (14.2,-1.0) .. (db.south west);
\node[lab] at (12.9,-1.05) {读状态回写};
\end{tikzpicture}
...
\subsection{前端业务辅助页面实现}

除认证与权限控制外，前端还实现了与成员激励、待办聚合和运营观察对应的辅助页面，用来补足管理侧与成员侧的日常使用体验。

\subsubsection{成员积分档案页面}

学生端积分档案页对应路由为 \texttt{/user/archive}。页面首先加载当前用户已加入的社团列表，用户选定社团后，再并行请求 \texttt{/clubs/\{id\}/members/me/archive} 和 \texttt{/clubs/\{id\}/members/me/point-records}，分别获取成员摘要信息与积分流水。这样既能展示成员在当前社团中的角色、加入时间、累计积分和月度积分，也能按时间顺序呈现积分增减原因，避免把不同社团的积分信息混在同一视图中。

社团管理员端的成员管理页则采用抽屉式档案查看方式。管理员点击某位成员后，页面并行请求 \texttt{/clubs/\{id\}/members/\{userId\}/archive} 和 \texttt{/clubs/\{id\}/members/\{userId\}/point-records}，在不离开成员列表的前提下直接查看该成员的积分画像与历史流水。相较于重新设计独立管理页面，这种做法更符合高频运营场景下“边看名单、边查档案”的使用习惯。

\subsubsection{统一待办中心与实时运营看板页面}

统一待办中心页面由管理员端和社团管理员端共用一套视图组件，区别仅在于入口路由分别为 \texttt{/admin/todos} 与 \texttt{/clubadmin/todos}。页面本身不再自行拼接多类待审数据，而是直接消费 \texttt{/api/dashboard/todos} 的聚合结果，并按分组展示待办总量、分组数量、代表性条目和对应跳转入口。这样做的实现重点在于将统计口径固定在后端，使同一账号在首页卡片、待办页和跳转目标页之间保持一致。

实时运营看板页面对应 \texttt{/admin/realtime}，在界面呈现上也可理解为管理端实时大屏。前端通过 \texttt{Promise.allSettled} 并发拉取 \texttt{/dashboard/stats} 与 \texttt{/stats/system}，在成功获取业务统计后绘制趋势图、状态分布图、压力仪表盘和实时折线图，并进一步按社团调用统计接口生成活跃排行。该页面展示的是社团数量、签到量、资源待审量、用户增长和社团活跃度等业务指标，用来支持演示和运营观察，而不是替代专门的系统监控平台。
...
\begin{enumerate}
  ...
  \item \textbf{文档切分}：使用 \texttt{RecursiveCharacterTextSplitter} 切分文本，当前参数为 \texttt{chunk\_size=1000}、\texttt{chunk\_overlap=200}；
...
这种混合推荐策略先用成员加入、活动报名、活动签到三类行为信号（5.0/2.0/3.0）和时间衰减构建用户偏好，再融合协同过滤与语义内容得分输出候选排序。对没有历史行为的新用户或单侧得分不可用的场景，系统会按社团热度回退，保证结果不断流；若推荐服务内部异常，当前接口会返回空列表，再由业务侧兜底展示。
