...
\begin{tikzpicture}[
  ...
  usecase/.style={draw, ellipse, minimum width=2.8cm, minimum height=0.8cm, align=center, font=\small},
  line/.style={-}
]
\draw[thick] (0,0) rectangle (12.4,9.2);
\node[above] at (6.2,9.2) {\textbf{校园社团管理系统}};

\node[actor] (visitor) at (-1.1,7.6) {};
\node[below] at (-1.1,7.2) {\small 访客};
\node[actor] (user) at (-1.1,4.9) {};
\node[below] at (-1.1,4.5) {\small USER};
\node[actor] (admin) at (13.5,6.9) {};
\node[below] at (13.5,6.5) {\small ADMIN};
\node[actor] (clubadmin) at (13.5,3.5) {};
\node[below] at (13.5,3.1) {\small CLUB\_ADMIN};

\node[usecase] (browse) at (2.9,7.6) {浏览社团/活动/公告};
\node[usecase] (auth) at (2.9,5.9) {登录/注册/找回密码};
\node[usecase] (apply) at (2.9,4.2) {活动报名/签到\\提交招新申请};
\node[usecase] (aiops) at (6.2,1.6) {AI 问答与推荐};

\node[usecase] (sysops) at (9.4,7.0) {全局审批与审计\\统计看板};
\node[usecase] (clubops) at (9.4,5.5) {社团管理\\成员与公告管理};
\node[usecase] (activityops) at (9.4,4.0) {活动发布\\招新审核};
\node[usecase] (resourceops) at (9.4,2.5) {资源与财务申请};
...
\draw[line] (admin) -- ++(-0.9,0) |- (clubops.north east);
\draw[line] (admin) -- ++(-1.5,0) |- (resourceops.north east);

\draw[line] (clubadmin) -- (resourceops.east);
\draw[line] (clubadmin) -- ++(-0.8,0) |- (activityops.east);
\draw[line] (clubadmin) -- ++(-1.4,0) |- (clubops.east);
\draw[line] (clubadmin) -- ++(-2.0,0) |- (aiops.east);
\end{tikzpicture}
\caption{系统总用例图}
\label{fig:usecase}
\end{figure}
